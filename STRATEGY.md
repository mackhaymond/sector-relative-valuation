...
An empirical backtest of the deviation signal is documented in `BACKTEST.md`. The current (Phase 3.5) numbers use point-in-time fundamentals from SimFin (filings filtered by `Publish Date <= as_of`) and per-snapshot walk-forward Ridge refitting; headline mean IC is **-0.0128** (|t| = 1.17, not statistically significant) and annualized long-short Sharpe is **0.054** net of 10 bps cost (negative at 25 bps). The earlier Phase 3 numbers (mean IC -0.0799, Sharpe 3.36) are preserved in `BACKTEST.md` §3's comparison table for methodological transparency but were inflated by look-ahead bias and should not be quoted. The honest read: the deviation signal does not predict forward 1-month returns at this universe / window / cost-level. The model output is best read as a structured description of today's relative valuation surface (a screen), not a forecast.

## 9. What would meaningfully extend this

In rough priority order (Phase 3.5 closed the PIT-fundamentals and walk-forward-Ridge items; the survivorship item is now the highest-leverage open work):

1. **Point-in-time index membership** (CRSP historical Russell 1000 files, or equivalent). At each backtest snapshot, evaluate only the names that were Russell 1000 members on that date. Eliminates the survivorship bias documented in `BACKTEST.md` §2.2 — now the dominant residual caveat.
2. **Longer sample period** — 10-15 years with regime-specific subsamples (2008-2009, 2020-Q1, 2022) — so the backtest can be evaluated under stress rather than only inside the 2023-2026 macro window the current run covers.
3. **Cross-vendor PIT fundamentals.** SimFin's free tier has a Financials coverage gap (`BACKTEST.md` §2.3) and updates infrequently. A second feed (Compustat point-in-time, S&P Capital IQ) would let banks rejoin the universe and tighten any per-sector signal estimate.
4. **Multiple forward-return horizons** (1mo / 3mo / 6mo / 12mo) computed simultaneously with bootstrap CIs. The current backtest locks the 1mo horizon as an anti-p-hacking guard; a principled multi-horizon test reported jointly would let an analyst see if signal accumulates at longer horizons.
...
6. A second data vendor for the volatile or missing fields (epsGrowth, cashFlowGrowth, banks' EBITDA margin) so Growth can broaden to a 2-3-metric composite and Quality stops silently dropping a metric for Financials.
7. **Sector-specific cost models** for the backtest. The current 10 bps round-trip is a flat textbook number; replacing it with name-level estimates (or at minimum a quintile-of-ADV-based scaling) would tighten the cost-sensitivity table in `BACKTEST.md` §3.
